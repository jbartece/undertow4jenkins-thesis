...
a~integračních testů, uložení spustitelné podoby aplikace do lokálního repozitáře 
        ...
        lokálního Maven repoitáře slouží parametr \texttt{install}.
...
\end{priklad}
    ...
    \section{Návrh způsobu integrace serveru Undertow} \label{secNavrh}
        ...
        \subsection{Zvolení způsobu integrace}
            ...
            zachován stávající stav tak, ktyrý byl popsán při analýze architektury (kapitola \ref{secArchitecture})
...
\chapter{Implementace}  \label{kapImpl}
 IN PROGRESS
    ...
    \section{Základní informace}
        ...
        Jak vyplývá z předchozích kapitol, tak program byl vytvořen v jazyce Java a pro správu a překlad projektu bylo nutné použít 
        program Maven. 
        ...
        komunitě Jenkins CI, tak byl tento projekt také verzován na serveru GitHub\footnote{adresa repozitáře} 
        a je uveřejněn pod licencí Apache License 2.0\footnote{link licence}, takže je \emph{open source} projektem.

        Na počátku integrace bylo nutné vytvořit kopie dvou repozitářů stávající implementace serveru Jenkins CI.
        Jedná se o repozitář s celým serverem\footnote{link fork jenkins} a také jeho modul \\\texttt{extras-executable-war}\footnote{link fork extras}.
        ...
        \medskip
        Jelikož hlavním cílem projektu je dosáhnou zpětné kompatibility s původní verzí servlet kontejneru,
        ...
        například při volbě šifrovacích protokolů pro nastavení komunikaci pomocí HTTPS nebo nastavení zaznamenávání
        příchozích požadavků.
        

    \section{Nahrazení původního servlet kontejneru}
        ...
        Jelikož je Jenkins CI překládán programem Maven, tak byla změněna závilost v souboru \texttt{pom.xml} 
...
\end{priklad}
    ...
    \section{Implementované funkce}
        ...
        V této kapitole jsou popsány kličové části aplikace a způsob jakým byly implementovány. Většina těchoto
        funkcionalit přímo koresponduje s uvedenými požadavky na zachování kompatibility, které jsou popsány v kapitole \ref{secKompatibilita}.


        %popis implementace jednotlivých částí funkčnosti - resourcemanager odkud bere veci, typ (mozna potom i caching), rozbalovani warka do webrootu
            %nebo do tmp, accesslogger implementovany jako handler na konci retezce handleru,     
        %mapovani starych classname na nove, uzivatelske vytvareni trid, webXML parser(podporovani jen 
        %casti ze specifikace verze 2.4, ktera je v jenkins, ale je to udelano obecne a pripadne rozsireni
        %je mozne bez zmeny architektury a nastavovani servlet
        %kontejneru a jeho porovnani s Jetty (ze tam je samozrejmosti) - v undertow by se casem mohlo taky vyskytnout (zdeleno pres irc),
        %pridavani prefixu k url adrese - vyreseno pridanim RedirectHandleru na zacatek weboveho serveru
        %classloading lib/classes pripadne slozky s knihovnami z adresare predaneho jako parametr, 
        %jak je nastaveno security, pridavani listeneru, implementace kontrol portu,
        %popis zpracovávání parametrů a přidání nových, příípadně změna default hodnot, nastavovani defaul mime-types ze souboru


        %3 maven projekty (repozitare), zmena v jenkins CI novych verzi extra-executables-war a noveho jarka s undertow4jenkins,
        %což je název, kterým byl pojmenován tento projekt
    ...
    \begin{figure}[h!t]
        \begin{center}
            ...
            \caption{Diagram balíků zachycující architekturu servlet kontejneru rozšířený o tři nejdůležitější třídy }
            \label{imgPackage}
        \end{center}
    \end{figure}

    
    %načítání parametrů Apache commons: http://commons.apache.org/proper/commons-cli/
    %logovaní slf4j a jako backend log4j - při přechodu by se dalo odstranit
    %jak distribuovano (maven), baleno do jar
    %package diagram - popis jednotlivých balíků a co vykonávají

    %zjistene problemy a jak resene?
    %využit parametr setIgnoreFlush a jeho popis


    \section{Problémy při implementaci}
        Implementace některých částí aplikace byla poměrně komplikovaná oproti původní verzi servlet kontejneru. Server Undertow 
        ...
        souboru \emph{web.xml} pomocí jediného volání metody.
        ...
        %proti jake verzi jenkins byl vyvoj provaden -- aktualni nejnovejsi v dobe pocatku vyvoje, ktery mel winstone-2.0
    ...
    k problemům s kompatibilitou např. při spouštění se staršími skripty, tak program zadání těchto
...
\chapter{Srovnání výkonu původní a~nové implementace}  \label{kapPerformance}
    ...
    %Zacatek grafu muze byt ovlivnen tim za jak dlouho se to spusti po startu Jenkins. Jenkns provadi
    %asynchronne ruzne cinnosti jako treba zjistovani novych aktualizaci. Tendence byla spustit to 
    %ve stejnou dobu, ale jelikoz se to spoustelo oddelene na dvou serverech, tak nebylo mozne dosahnout uplne
    %stejneho okamziku

    \section{Zhodnocení přínosu pro komunitu Jenkins CI}
    %výkonnost, čitelnější kód, odstranění nevyužívaných věcí, 
    %-je potřeba další testování a potenciálně úpravy než se provede začlenění
    %zmínit velikost archivu
