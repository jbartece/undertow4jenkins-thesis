...
\chapter{Implementace}  \label{kapImpl}
    ...
    %nějaké uvední do toho, oecné věci o implementaci, ve sporných místech hledání ve starých zdrojácích
    %a snaha zachovat význam implementace
...
%                    samotné zdrojové kódy serveru, abych vše správně pochopil a využil.
    ...
    %jak prelozit a spustit
    %posloupnost maven installu, zpusob spusteni programu
    %vytvorene 6 zakladnich unit testu a zkousena aplikace fungovala, nelze vyuzit integracni testy celeho jenkins, protoze
    %ten nebezi na servlet kontejneru pro celou aplikaci, ale na nejake mensi jetty
    %licence

    \section{Omezení aplikace}
    %Program byl otestovan a jevil se plne funkce, ale je tam nekolik uprav v parametrech 
    
    %je je mozne zadat, ale bude vypsana informativni hlaska, ze nejsou podporovane, ale program bude fungovat

    %ignorovane parametry      
    %nastavovano spolecne s tim, ze prednost ma nastaveni u http, nastaveni mozne pouze pro cely server
    %nebo to uplne rucne provest sestaveni serveru, coz by slo, ale pro pripadny nasledujici vyvo by to byla komplikace
    %ponechano aktualne jako nastavovani dohromady (naprikald pri pridani podpory SPDY by se kod musel prepracovat)
    ...
    %spdy;          -- aktualne neni podpora v undertow ale brzo bude

    %webappsDir;    -- nedava smysl, vice instanci jenkins nemuze bezet na jednom stroji (pozustatek)
    %hostsDir;
    
    %Npouzivane uz ani ve winstone 2.0
    %httpDoHostnameLookups;
    %httpsDoHostnameLookups;
    %handlerCountStartup;
    %logThrowingLineNo;

    %V jetty se vlakna vytvarela rucen a nasledne predavala k vykonani.
    %V underotw tato moznost nebyla a je to ponechano na serveru
    %handlerCountMaxIdle;

    %Naprosto nezasadni - ponechano pro pripadny nasledujici vyvoj
    %logThrowingThread;
    %debug;
